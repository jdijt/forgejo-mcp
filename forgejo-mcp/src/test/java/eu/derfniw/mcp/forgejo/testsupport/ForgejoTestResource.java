...
public class ForgejoTestResource implements QuarkusTestResourceLifecycleManager {
    ...
    public static final String DEMO_REPO_NAME = "demo-repo";
    public static final String DEMO_REPO_OWNER = TEST_USERNAME;
    ...
    private static volatile String testUserPat;
    ...
    /** PAT issued to {@link #TEST_USERNAME} during bootstrap; suitable as a bearer for the Forgejo REST API. */
    public static String testUserPat() {
        return testUserPat;
    }
    ...
    private void createDemoRepo() throws Exception {
        ObjectMapper mapper = new ObjectMapper();
        byte[] body = mapper.writeValueAsBytes(
                new CreateRepoRequest(DEMO_REPO_NAME, "Fixture repo for MCP broker tests.", true, "main", "MIT"));
        String basicAuth = Base64.getEncoder()
        ...
        try (HttpClient http =
            ...
            if (resp.statusCode() < 200 || resp.statusCode() >= 300) {
                throw new RuntimeException(
                        "Demo repo create failed: HTTP " + resp.statusCode() + " body=" + resp.body());
            }
        }
    }

    private String createTestUserPat() throws Exception {
        ObjectMapper mapper = new ObjectMapper();
        byte[] body = mapper.writeValueAsBytes(
                new CreateTokenRequest("mcp-test-pat", List.of("read:repository", "read:issue", "read:user")));
        String basicAuth = Base64.getEncoder()
                .encodeToString((TEST_USERNAME + ":" + TEST_PASSWORD).getBytes(StandardCharsets.UTF_8));

        HttpRequest req = HttpRequest.newBuilder()
                .uri(URI.create(baseUrl + "/api/v1/users/" + TEST_USERNAME + "/tokens"))
                .header("Authorization", "Basic " + basicAuth)
                .header("Content-Type", "application/json")
                .header("Accept", "application/json")
                .timeout(Duration.ofSeconds(15))
        ...
        try (HttpClient http =
            ...
            HttpResponse<String> resp = http.send(req, HttpResponse.BodyHandlers.ofString());
            if (resp.statusCode() < 200 || resp.statusCode() >= 300) {
                throw new RuntimeException("PAT create failed: HTTP " + resp.statusCode() + " body=" + resp.body());
            }
            CreateTokenResponse parsed = mapper.readValue(resp.body(), CreateTokenResponse.class);
            return parsed.sha1();
        }
    }

    private record CreateRepoRequest(
            String name, String description, boolean auto_init, String default_branch, String license) {}

    private record CreateTokenRequest(String name, List<String> scopes) {}

    @JsonIgnoreProperties(ignoreUnknown = true)
    private record CreateTokenResponse(String sha1) {}
}
