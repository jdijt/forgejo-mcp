...
public class CimdResolver {
    ...
    public CimdDocument resolve(String clientIdUrl) {
        ...
        try {
            ...
            throw new BadRequest("client_id is not a valid URL: " + clientIdUrl, e);
        }
        if (uri.getScheme() == null || uri.getHost() == null) {
            throw new BadRequest("client_id URL must be absolute: " + clientIdUrl);
        }
        brokerConfig.cimd().allowedHosts().ifPresent(allowedHosts -> {
            if (!allowedHosts.contains(uri.getHost())) {
                throw new BadRequest("CIMD Host not allowed");
            }
        });
        ...
        try {
            ...
            throw new UpstreamFailure("CIMD fetch was interrupted: " + clientIdUrl, e);
        }

        if (resp.statusCode() != 200 && resp.statusCode() < 500) {
            throw new BadRequest("CIMD fetch returned HTTP " + resp.statusCode() + " for " + clientIdUrl);
        ...
        try {
            ...
            Log.warnf(
                    ...
                    resp.body());
            throw new BadRequest("CIMD body is not valid JSON: " + clientIdUrl, e);
        }

        if (!doc.clientId().equals(clientIdUrl)) {
            throw new BadRequest("CIMD client_id does not match URL: doc=" + doc.clientId() + " url=" + clientIdUrl);
        }
        if (doc.redirectUris().isEmpty()) {
            throw new BadRequest("CIMD must declare at least one redirect_uri: " + clientIdUrl);
        }
